...
float3 satRGBToHSL(float3 color);
float SatHueToRGB(float f1, float f2, float hue);
float3 SatHSLToRGB(float3 hsl);
...
float4 CompositeTopAndBottom(thread float4 & bottom, thread float4 & top, thread float & topAlpha, thread MSLCompModeFragData & fragData)
{
	...
	returnMe.rgb = SatHSLToRGB(returnMe.rgb);
...
float3 SatHSLToRGB(float3 hsl)
{
	...
	else
	{
		...
		rgb.r = SatHueToRGB(f1, f2, hsl.x + (1.0/3.0));
		rgb.g = SatHueToRGB(f1, f2, hsl.x);
		rgb.b= SatHueToRGB(f1, f2, hsl.x - (1.0/3.0));
	}
