...
float3 hueRGBToHSL(float3 color);
float HueHueToRGB(float f1, float f2, float hue);
float3 HueHSLToRGB(float3 hsl);
...
float4 CompositeTopAndBottom(thread float4 & bottom, thread float4 & top, thread float & topAlpha, thread MSLCompModeFragData & fragData)
{
	...
	returnMe.rgb = HueHSLToRGB(returnMe.rgb);
...
float3 HueHSLToRGB(float3 hsl)
{
	...
	else
	{
		...
		rgb.r = HueHueToRGB(f1, f2, hsl.x + (1.0/3.0));
		rgb.g = HueHueToRGB(f1, f2, hsl.x);
		rgb.b= HueHueToRGB(f1, f2, hsl.x - (1.0/3.0));
	}
