...
float3 colorRGBToHSL(float3 color);
float ColorHueToRGB(float f1, float f2, float hue);
float3 ColorHSLToRGB(float3 hsl);
...
float4 CompositeTopAndBottom(thread float4 & bottom, thread float4 & top, thread float & topAlpha, thread MSLCompModeFragData & fragData)
{
	...
	returnMe.rgb = ColorHSLToRGB(returnMe.rgb);
...
float3 ColorHSLToRGB(float3 hsl)
{
	...
	else
	{
		...
		rgb.r = ColorHueToRGB(f1, f2, hsl.x + (1.0/3.0));
		rgb.g = ColorHueToRGB(f1, f2, hsl.x);
		rgb.b= ColorHueToRGB(f1, f2, hsl.x - (1.0/3.0));
	}
