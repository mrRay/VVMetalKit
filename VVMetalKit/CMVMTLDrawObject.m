...
- (void) executeInRenderEncoder:(id<MTLRenderCommandEncoder>)inRenderEnc textureArgumentEncoder:(id<MTLArgumentEncoder>)inTexArgEnc commandBuffer:(id<MTLCommandBuffer>)inCB	{
	...
	//if (self.images.count > 0)	{
		...
		//	if there aren't any images, still encode a VALID resident texture (empty black) into slot 0.
		//	Tier-1 argument-buffer GPUs (e.g. Intel HD 5000) resolve every encoded slot at draw finalize even
		//	when the shader won't sample it, so a recycled pool buffer left un-encoded holds a stale handle and
		//	faults. Mirrors the emptyBlackTexture fallback in the non-argument-encoder variant above.
		if (sortedImages.count < 1)	{
			[inTexArgEnc setArgumentBuffer:texArrayBuffer.buffer offset:0];
			id<MTLTexture>		emptyTex = VVMTLPool.global.emptyBlackTexture.texture;
			if (emptyTex != nil)	{
				[inTexArgEnc setTexture:emptyTex atIndex:0];
				[inRenderEnc useResource:emptyTex usage:MTLResourceUsageRead stages:MTLRenderStageFragment];
			}
		}
		//	make an argument encoder, populate it with the array of textures
			...
			for (id<VVMTLTextureImage> image in sortedImages)	{
				...
				//	an image can be missing its texture if creation failed in the pool- encode the empty-black
				//	fallback into its slot rather than skipping it: the recycled pool buffer is un-zeroed, so an
				//	un-encoded slot holds a stale handle a Tier-1 driver faults on. Keeps slot alignment too.
